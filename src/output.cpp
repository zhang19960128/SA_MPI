...
void write_opt_parameter(std::fstream& fs){
	...
	for(size_t i=0;i<control::pair_num;i++){
	  for(size_t j=0;j<12;j++){
			fs<<std::setw(8)<<std::fixed<<std::setprecision(7)<<control::bvvmatrix[i][j]<<"\t";
		}
		fs<<std::endl;
	}
	...
	for(size_t i=0;i<size;i++){
		fs<<std::setw(8)<<std::fixed<<std::setprecision(7)<<control::charge[i]<<"\t";
	}
	fs<<std::endl;
	...
	for(size_t i=0;i<control::pair_num;i++){
	  for(size_t j=0;j<12;j++){
			fs<<std::setw(8)<<std::fixed<<std::setprecision(7)<<control::bvvmatrixmap[i][j]*control::vm[tick]<<"\t";
	...
	fs<<std::endl;
	for(size_t i=0;i<control::site_name.size();i++){
		fs<<std::setw(8)<<std::fixed<<std::setprecision(7)<<control::vm[tick]<<"\t";
		tick++;
	}
