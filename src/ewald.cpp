...
void box::computelong(double accuracy_relative){
 ...
 double err;
  ...
  double rij;
       ...
       std::cout<<"S1[i] S2[i] S3[i]"<<s1[i]<<" "<<s2[i]<<" "<<s3[i]<<std::endl;
			 fx[i]=0.00;
			 fy[i]=0.00;
			 fz[i]=0.00;
    ...
    std::cout<<"the short range energy is: "<<ShortRange<<std::endl;
    for(int  i=0;i<size;i++){
         ...
         for(int i=-max_g1;i<=max_g1;i++){
          for(int j=-max_g2;j<=max_g2;j++){
            for(int k=-max_g3;k<=max_g3;k++){
              ...
              fy[m]=fy[m]+2*prefactor*q[m]*q[t]*2*PI*(i*g11+j*g21+k*g31)*(sinkdotrm*coskdotrj-coskdotrm*sinkdotrj);
              fz[m]=fz[m]+2*prefactor*q[m]*q[t]*2*PI*(i*g11+j*g21+k*g31)*(sinkdotrm*coskdotrj-coskdotrm*sinkdotrj);
            }
          }
         }
      }
